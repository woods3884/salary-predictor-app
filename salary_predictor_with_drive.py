...
if sheet_id and sheet_id != "ここに貼り付け":
    try:
        ...
        st.success("✅ Googleスプレッドシートからの読み込み成功")
...
# Google Drive へアップロード機能（任意でフォルダIDを指定可能）
with st.expander("☁️ Google Driveへ自動保存"):
    upload = st.button("📤 Driveにアップロード")
    folder_id = st.text_input("DriveフォルダID（空欄でマイドライブ直下）", "")

    if upload:
        try:
            file_id = upload_pdf_to_drive(pdf_buffer, "給与レポート.pdf", folder_id if folder_id else None)
            st.success(f"✅ アップロード成功！ファイルID：{file_id}")
            st.markdown(f"[📁 Driveで開く](https://drive.google.com/file/d/{file_id}/view)", unsafe_allow_html=True)
        except Exception as e:
